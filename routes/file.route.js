  ...
  if (req.body.name && req.body.data) {
    try {
      ...
      res
        .status(201)
        .send(
          `File ${req.body.name} has been created. Use "GET" on /file/readFile/${req.body.name} to read the file contents.`
        );
    } catch (err) {
  ...
  if (req.body.name && req.body.data) {
    try {
      ...
      res
        .status(201)
        .send(
          `The file has been updated. Use "GET" on /file/readFile/${req.body.name} to read the file contents.`
        );
    } catch (err) {
  ...
  if (req.body.name) {
    try {
      ...
      res
        .status(201)
        .send(
          `The file "${req.body.name}" is delete. Use "GET" on /file/show to see available files.`
        );
    } catch (err) {
